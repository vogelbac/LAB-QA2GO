function LCOD_sl10_t1_neu()
global img1 dim1_T1 dim2_T1 score_sl10_t1 dir_results
...
for column = pos2ob:pos1un,
    ...
    if pos_row_dim > 0
	    ...
	    im_circle = img1(column,dim2_circle,9);
...
for phi6 = 182:192
    ...
    for rn = 1:round(r_best)        
        ...
        if a6x(rn,phi_n) == 0
           a6x(rn,phi_n) = xc6+44.5;
        end
        A6(rn,phi_n) = imsheet(ceil(a6x(rn,phi_n)),round(b6y(rn,phi_n)),1);
    end 
    phi_n = phi_n+1;
end
for i = 1:length(A6)-1
...
set(f,'Name','LCOD_sl10_t1');
print(f,[dir_results 'LCOD_sl10_t1'],'-dpng');
...
score_sl10_t1 = count1+count2+count3+count4+count5+count6+count7+count8+count9+count10
