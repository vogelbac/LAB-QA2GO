function LCOD_sl9_neu()
global img2 dim1_T2 dim2_T2 score_sl9 dir_results
...
[mx1_li loc_mx1_li] = max(abldim1(60:85)); 
[mn1_li loc_mn1_li] = min(abldim1(60:85)); 
[mx1_re loc_mx1_re] = max(abldim1(160:180)); 
[mn1_re loc_mn1_re] = min(abldim1(160:180)); 
pos1li = loc_mn1_li+60
pos2li = loc_mx1_li+60
pos1re = loc_mn1_re+160
pos2re = loc_mx1_re+160
abldim2 = diff(sumdim2);
[mx2_ob loc_mx2_ob] = max(abldim2(70:100)); 
[mn2_ob loc_mn2_ob] = min(abldim2(70:100)); 
[mx2_un loc_mx2_un] = max(abldim2(160:190)); 
[mn2_un loc_mn2_un] = min(abldim2(160:190)); 
pos1ob = loc_mn2_ob+70
pos2ob = loc_mx2_ob+70
pos1un = loc_mn2_un+160
pos2un = loc_mx2_un+160
...
greyimage = 255*((imsheet1(:,:,1))./((mx_imsheet1*1.4)-mn_imsheet1));
figure(200)
imagesc(greyimage)
...
imsheet = imsheet1.*imsheet2.*imsheet3;
...
for phi3 = 69:79
    ...
    for rn = 1:round(r_vert)        
        ...
        A3(rn,phi_n) = imsheet1(round(a3x(rn,phi_n)),round(b3y(rn,phi_n)),1);
...
for phi6 = 172:182
    ...
    for rn = 1:round(r_vert)        
        ...
        if a6x(rn,phi_n) == 0
           a6x(rn,phi_n) = xc6+46.5;
        end
        %A6(rn,phi_n) = imsheet1(round(a6x(rn,phi_n)),round(b6y(rn,phi_n)),1);
        A6(rn,phi_n) = imsheet1(ceil(a6x(rn,phi_n)),round(b6y(rn,phi_n)),1);
...
score_sl9 = count1+count2+count3+count4+count5+count6+count7+count8+count9+count10
...
set(f,'Name','LCOD_sl9_t2');
print(f,[dir_results 'LCOD_sl9_t2'],'-dpng');
