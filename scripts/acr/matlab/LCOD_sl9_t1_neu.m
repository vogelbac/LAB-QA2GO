function LCOD_sl9_t1_neu()
global img1 dim1_T1 dim2_T1 score_sl9_t1 dir_results
FoV_dim = 250;
...
Objs(:,:) = img1(:,:,8).*mask(:,:);
imsheet_nothresh(:,:) = Objs(pos2ob:pos1un,pos2li:pos1re);
...
imsheet(:,:) = Objs(pos2ob:pos1un,pos2li:pos1re);
...
for phi1 = 356:366
    ...
    for rn = 1:round(r_best)      
        ...
        A1(rn,phi_n) = imsheet(round(a1x(rn,phi_n)),round(b1y(rn,phi_n)));
    end 
    phi_n = phi_n+1;
end
...
for phi2 = 34:44
    ...
    for rn = 1:round(r_best)        
        ...
        A2(rn,phi_n) = imsheet(round(a2x(rn,phi_n)),round(b2y(rn,phi_n)));
    end 
    phi_n = phi_n+1;
end
for i = 1:length(A2)-1
...
for phi3 = 69:79
    ...
    for rn = 1:round(r_best)        
        ...
        A3(rn,phi_n) = imsheet(round(a3x(rn,phi_n)),floor(b3y(rn,phi_n)));
    end 
    phi_n = phi_n+1;
end
for i = 1:length(A3)-1
...
for phi4 = 105:115
    ...
    for rn = 1:round(r_best)        
        ...
        A4(rn,phi_n) = imsheet(round(a4x(rn,phi_n)),floor(b4y(rn,phi_n)));
    end 
    phi_n = phi_n+1;
end
for i = 1:length(A4)-1
...
for phi5 = 140:150
    ...
    for rn = 1:round(r_best)        
        ...
        A5(rn,phi_n) = imsheet(ceil(abs(a5x(rn,phi_n))),round(b5y(rn,phi_n)));
    end 
    phi_n = phi_n+1;
end
for i = 1:length(A5)-1
...
for phi6 = 174:184
    ...
    for rn = 1:round(r_best)        
        ...
        a6x(rn,phi_n) = xc6+round(r_best);
        b6y(rn,phi_n) = yc6+r_best;
        if a6x(rn,phi_n) <= 0
           a6x(rn,phi_n) = 1;
        end
        A6(rn,phi_n) = imsheet(ceil(a6x(rn,phi_n)),round(b6y(rn,phi_n)));
    end 
    phi_n = phi_n+1;
end
for i = 1:length(A6)-1
...
for phi7 = 207:217
    ...
    for rn = 1:round(r_best)        
        ...
        A7(rn,phi_n) = imsheet(round(a7x(rn,phi_n)),round(b7y(rn,phi_n)));
    end 
    phi_n = phi_n+1;
end
for i = 1:length(A7)-1
...
for phi8 = 243:253
    ...
    for rn = 1:round(r_best)        
        ...
        A8(rn,phi_n) = imsheet(round(a8x(rn,phi_n)),ceil(abs((b8y(rn,phi_n)))));
    end 
    phi_n = phi_n+1;
end
for i = 1:length(A8)-1
...
for phi9 = 282:292
    ...
    for rn = 1:round(r_best)        
        ...
        A9(rn,phi_n) = imsheet(ceil(abs(a9x(rn,phi_n))),ceil(abs((b9y(rn,phi_n)))));
    end 
    phi_n = phi_n+1;
end
for i = 1:length(A9)-1
...
for phi10 = 318:328
    ...
    for rn = 1:round(r_best)        
        ...
        A10(rn,phi_n) = imsheet(round(abs(a10x(rn,phi_n))),round(abs((b10y(rn,phi_n)))));
    end 
    phi_n = phi_n+1;
end
...
set(f,'Name','LCOD_sl9_t1');
print(f,[dir_results 'LCOD_sl9_t1'],'-dpng');
...
score_sl9_t1 = count1+count2+count3+count4+count5+count6+count7+count8+count9+count10
