...
%Interpolation
[x,y]=meshgrid(1:dim1_T1);  %Bildgr��e quadratisch dim1=dim2 (256*256)
[xi,yi]=meshgrid(1:0.1:dim1_T1);
...
roi_UR = roi_up(45:145,85:185,1);
roi_LL = roi_up(120:220,15:115,1);
...
roi_CR = roi_center(45:135,90:175,1);
roi_CL = roi_center(120:200,15:110,1);
...
%end
%if length(maxtab_ver_CL) > 2,
elseif length(maxtab_ver_CL) > 2,
    a_ver_CL = maxtab_ver_CL(1);
    b_ver_CL = maxtab_ver_CL(2);
    c_ver_CL = maxtab_ver_CL(3);
    d_ver_CL = c_ver_CL+20;
    d_ver_CL = c_ver_CL+18;
%end
%if length(maxtab_ver_CL) > 1,
elseif length(maxtab_ver_CL) > 1,
    a_ver_CL = maxtab_ver_CL(1);
    b_ver_CL = maxtab_ver_CL(2);
    c_ver_CL = b_ver_CL+20;
    d_ver_CL = c_ver_CL+20;
    c_ver_CL = b_ver_CL+18;
    d_ver_CL = c_ver_CL+18;
end
temp_size = size(roi_CL);

if d_ver_CL > temp_size(2)
    d_ver_CL = temp_size(2);
end
...
roi_BR = roi_bottom(60:135,90:170,1);
roi_BL = roi_bottom(120:200,20:110,1);
...
if d_ver_BL > temp_size(1)
    d_ver_BL = temp_size(1);
end
...
bild = figure(3);
%set(gcf,'numbertitle','off','Name','Spatial resolution: Hole pair arrays');
subplot(3,2,1)
imagesc(roi_UR);
colormap(gray);
colorbar();
...
fname_result = sprintf('Resolution_Results_%s.txt',nameT1);

cd(dir_results);
%Speichern der Ergebnisse in  Textfile und png Format
%Bild speichern
resT1bild = sprintf('resolution_%s',nameT1); 
...
fprintf(global_result_file,'\n#2\nErgebnisse Spatial Resolution T1:\n');
