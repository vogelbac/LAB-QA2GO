...
y1_bottom = round(cut2+(dim1_T2-cut2)/2-h_rec/2);
...
x1_right = round(cut4+(dim2_T2-cut4)/2-h_rec/2);
x2_right = x1_right+h_rec;
y1_right = round(ym-w_rec/2);
y2_right = y1_right+w_rec;

roi_top = img2(y1_top:y2_top,x1_top:x2_top,7);
mean_top = mean(mean(roi_top))
roi_bottom = img2(y1_bottom:y2_bottom,x1_bottom:x2_bottom,7);
mean_bottom = mean(mean(roi_bottom))
roi_left = img2(y1_left:y2_left,x1_left:x2_left,7);
mean_left = mean(mean(roi_left))
roi_right = img2(y1_right:y2_right,x1_right:x2_right,7);
...
rectangle('position',[round(xm-w_rec/2) round(cut2+(dim1_T2-cut2)/2-h_rec/2) w_rec h_rec],'FaceColor',[0 0 1],'EdgeColor','none');
rectangle('position',[round(cut3/2-h_rec/2) round(ym-w_rec/2) h_rec w_rec],'FaceColor',[0 0 1],'EdgeColor','none');
rectangle('position',[round(cut4+(dim2_T2-cut4)/2-h_rec/2) round(ym-w_rec/2) h_rec w_rec],'FaceColor',[0 0 1],'EdgeColor','none');
...
set(bild, 'PaperUnits', 'centimeter', ...
   ...
   'PaperSize', [12, 9]);
sl_pos_bild = sprintf('Ghosting_%s',nameT2); 
print( gcf, '-dpng', sl_pos_bild );

fname_result = sprintf('Ghosting_T2_Results.txt');
%Parameter speichern
